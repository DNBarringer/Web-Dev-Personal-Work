...
function initializeForm() {
  ...
  LoadHTMLFile("CakeType", "CakeTypeSelection.html");

  LoadHTMLFile("invoiceBox", "Invoice.html");
}
      ...
      switch (ajaxCall.status) {
        case 200:
          ...
          if (element && element.innerHTML !== undefined) {
            element.innerHTML = ParseTextAsXML(ajaxCall.responseText,'body');
          }
          else {
...
const replacementAreaId = "CakeSize";


function CakeChoiceRound() {
    var ajaxCall = new XMLHttpRequest();
    // Setup the callback function
    ajaxCall.onreadystatechange = function () {
      if (ajaxCall.readyState == 4 && ajaxCall.status == 200) {
        document.getElementById(replacementAreaId).innerHTML = ajaxCall.responseText;
      }
    };
    ...
    LoadHTMLFile("AdditionalChoices", "OptionalAddOns.html");

}

function CakeChoiceSheet() {
    var ajaxCall = new XMLHttpRequest();
    // Setup the callback function
    ajaxCall.onreadystatechange = function () {
      if (ajaxCall.readyState == 4 && ajaxCall.status == 200) {
